(() => {
  ...
  const candidate = o => {
    ...
    for (const [field, names] of Object.entries(ALIASES)) {
      if (['asset', 'price', 'bid', 'ask'].includes(field)) continue;
      const v = pick(o, names);
      if (v == null) continue;
      if (['open', 'high', 'low', 'close', 'payout'].includes(field)) {
        const n = num(v);
        if (n != null) c[field] = n;
  ...
  const scan = (data, meta = {}) => {
    ...
    while (stack.length && seen < 600) {
      ...
      if (Array.isArray(v)) {
        for (let i = Math.min(v.length, 120) - 1; i >= 0; i--) stack.push({ v: v[i], d: d + 1 });
        continue;
      }
      const c = candidate(v);
    ...
    flushTimer.id = setTimeout(() => {
      ...
      window.postMessage({
        ...
        payload: {
          ...
          endpoints: [...stats.endpoints].slice(-20),
          keys: [...stats.keys].slice(0, 120),
          candidates: [...stats.assets.values()].slice(0, 100),
          candidateCount: stats.assets.size,
    ...
    window.fetch = async function(...args) {
      ...
      try {
        ...
        if (/json|text|javascript/i.test(ct)) {
          const clone = r.clone();
          clone.text().then(t => {
            if (t.length <= 262144) record('fetch', u, t);
          }).catch(() => {});
        }
      } catch {}
      return r;
      ...
      this.addEventListener('load', () => {
        try {
          ...
          if (this.responseType === '' || this.responseType === 'text') {
            ...
            if (t.length <= 262144) record('xhr', this.__atsUrl, t);
