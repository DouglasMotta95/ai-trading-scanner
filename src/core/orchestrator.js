...
function builderKey(snapshot = {}) {
  return `${clean(snapshot.platformId || 'unknown')}|${clean(snapshot.asset)}|${clean(snapshot.analysisTimeframe || snapshot.timeframe || 'M1')}`;
}
...
function baseSignal({
  ...
  uiState = 'ANALYZING_MARKET', analytics = {}, waitingFor = null, stability = null, regime = null
} = {}) {
