...
function simulate(candles,profile,threshold,timeframe='M1',payout=.85){
  ...
  for(let i=30;i<candles.length-1;i++){
    const window=candles.slice(Math.max(0,i-80),i+1),analysis=analyzeCandles(window),direction=analysis.direction;
    if(!direction)continue;
    const structure=marketStructure(window);
    const review=weightedConfidence({direction,profile,structure,indicators:analysis.indicators,candles:window,confirmations:analysis.score>=70?4:2,totalConfirmations:6,correlation:{score:70},news:{unknown:true}});
    if(review.score<threshold)continue;
    const entry=n(candles[i]?.close),exit=n(candles[i+1]?.close);if(entry==null||exit==null)continue;
    const setup=review.score>=85?'A+':tfType(timeframe)==='scalping'?'scalping':'normal';
    trades.push({at:n(candles[i]?.time??candles[i]?.timestamp),direction,entry,exit,score:review.score,setup,outcome:outcome(direction,entry,exit)});
  }
...
export function runBacktest(candles=[],options={}){
  ...
  if(rows.length<32)return{ok:false,error:'insufficient_history',required:32,available:rows.length,generatedAt:Date.now()};
  const profiles={
  ...
  return{ok:true,generatedAt:Date.now(),candles:rows.length,timeframe,payout,profiles,bySetup,summary:profiles.moderate};
}
