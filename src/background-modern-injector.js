...
function executeScriptCompat(details) {
  return new Promise((resolve, reject) => {
    ...
    const finish = (error, result) => {
      if (settled) return;
      settled = true;
      if (error) reject(error); else resolve(Array.isArray(result) ? result : []);
    };
...
function inspectFrame() {
  const describe = value => {
    try {
      ...
      return {
        ...
        host: String(url.hostname || '').toLowerCase().replace(/\.$/, '').slice(0, 120),
  ...
  if (window === window.top) {
    for (const frame of [...document.querySelectorAll('iframe')].slice(0, 16)) {
      ...
      row.srcdoc = frame.hasAttribute('srcdoc');
      hints.push(row);
    }
  }
  let referrerHost = '';
  try { referrerHost = new URL(document.referrer || '').hostname.toLowerCase().replace(/\.$/, '').slice(0, 120); } catch {}
  return {
    protocol: String(location.protocol || '').slice(0, 16),
    host: String(location.hostname || '').toLowerCase().replace(/\.$/, '').slice(0, 120),
    referrerHost,
...
const frameRows = results => (Array.isArray(results) ? results : []).slice(0, 30).map(row => ({
  frameId: Number.isFinite(Number(row?.frameId)) ? Number(row.frameId) : null,
  protocol: String(row?.result?.protocol || '').slice(0, 16),
  host: String(row?.result?.host || '').slice(0, 120),
  referrerHost: String(row?.result?.referrerHost || '').slice(0, 120),
  isTop: row?.result?.isTop === true,
  readyState: String(row?.result?.readyState || '').slice(0, 24),
  iframeHints: Array.isArray(row?.result?.iframeHints) ? row.result.iframeHints.slice(0, 16) : []
}));

async function probeFrames(tabId) {
  const target = { tabId, allFrames: true };
  try {
    const result = await executeScriptCompat({ target, func: inspectFrame, world: 'ISOLATED' });
    return { ok: true, mode: 'isolated-world', frameCount: result.length, frames: frameRows(result), error: '' };
  } catch (firstError) {
    try {
      const result = await executeScriptCompat({ target, func: inspectFrame });
      return { ok: true, mode: 'default-world-fallback', frameCount: result.length, frames: frameRows(result), firstError: cleanError(firstError), error: '' };
    } catch (secondError) {
      return { ok: false, mode: 'failed', frameCount: 0, frames: [], firstError: cleanError(firstError), error: cleanError(secondError) };
    }
  }
}
...
async function injectFile(tabId, file, world) {
  const target = { tabId, allFrames: true };
  try {
    const result = await executeScriptCompat({ target, files: [file], world });
    return { file, world, ok: true, mode: `${String(world).toLowerCase()}-world`, frameCount: result.length, error: '' };
  } catch (firstError) {
    if (world === 'ISOLATED') {
      try {
        const result = await executeScriptCompat({ target, files: [file] });
        return { file, world, ok: true, mode: 'default-world-fallback', frameCount: result.length, firstError: cleanError(firstError), error: '' };
      } catch (secondError) {
        return { file, world, ok: false, mode: 'failed', frameCount: 0, firstError: cleanError(firstError), error: cleanError(secondError) };
      }
    }
...
async function recordInjection(tabId, startedAt, probe, rows) {
  ...
  const isolated = rows.filter(row => row.world === 'ISOLATED');
  const main = rows.filter(row => row.world === 'MAIN');
  ...
  const telemetry = {
    ...
    isolated: {
      ...
      ok: isolated.filter(row => row.ok).length,
      fallback: isolated.filter(row => row.mode === 'default-world-fallback').length
    },
    main: { total: main.length, ok: main.filter(row => row.ok).length },
    failures
  };
  await updateScannerState(state => ({
    ...state,
    diagnostics: { ...(state.diagnostics || {}), runtimeInjection: telemetry }
  })).catch(() => {});
  return telemetry;
}
...
async function inject(tabId) {
  ...
  if (!tabId || !chrome.scripting?.executeScript) {
    const probe = { ok: false, mode: 'scripting-api-unavailable', frameCount: 0, frames: [], error: 'chrome.scripting.executeScript unavailable' };
    await recordInjection(tabId || null, startedAt, probe, []);
    return false;
  }
  ...
  const isolated = [
    ...
    'src/content/page-world-bootstrap.js',
  ...
  const probe = await probeFrames(tabId);
  const rows = [];
...
// Manifest content scripts own normal page-load injection. This hook also performs
// compatibility recovery and records exactly what Quetta accepted or rejected.
