(() => {
  ...
  function performanceReport(rows = [], state = {}, uiPrefs = {}) {
    const safeRows = (Array.isArray(rows) ? rows : []).slice(-2000).map(row => ({
      ...
      confirmation: clean(row?.confirmation || 'SIMPLES', 16).toUpperCase() === 'EXIGENTE' ? 'EXIGENTE' : 'SIMPLES',
      score: num(row?.score),
    ...
    return [
      ...
      '',
      'POR CONFIRMAÇÃO',
      groupLines(safeRows, row => row.confirmation, list => breakEvenForRows(list)),
      '',
      'POR FAIXA DE SCORE',
