...
function renderLicense(s={}){
  const l=s.license||{},active=l.status==='active',dev=!!l.development,trial=l.isTrial||l.plan==='trial',limit=trial&&l.totalLimit!=null?l.totalLimit:l.dailyLimit,used=trial&&l.totalLimit!=null?Number(l.usedTotal)||0:Number(l.usedToday)||0;
  $('licenseCard').classList.toggle('active',active);$('licenseCard').classList.toggle('blocked',!active);$('activationBox').hidden=active;
  $('licenseHealth').textContent=active?(dev?'DEV':String(l.planLabel||l.plan||'ATIVA').toUpperCase()):String(l.status||'INATIVA').toUpperCase();
  $('planBadge').textContent=dev?'DEV':active?String(l.planLabel||l.plan||'ATIVA').toUpperCase():'LICENÇA';
  $('licenseTitle').textContent=dev?'Modo de desenvolvimento':active?'Licença ativa':l.status==='expired'?'Licença expirada':l.status==='limit'?'Limite do plano atingido':l.status==='device_locked'?'Licença vinculada a outro aparelho':'Ativação necessária';
  $('licenseText').textContent=dev?'Build local liberada para desenvolvimento.':active?`${l.planLabel||l.plan||'Plano'} ativo${l.expiresAt?` • vence ${new Date(l.expiresAt).toLocaleDateString('pt-BR')}`:''}.`:l.status==='expired'?'Esta licença precisa ser renovada.':l.status==='device_locked'?'Esta licença já está vinculada a outro aparelho. A troca precisa ser liberada pelo administrador.':l.error==='backend_not_configured'?'Backend de licenças não configurado.':'Informe sua chave de licença para continuar.';
  $('licenseUsage').textContent=active?usageText(l):'Uso: —';
  $('licenseDevice').textContent=dev?'Aparelho: DEV':active?(l.deviceLocked||l.devices>0?'Aparelho: VINCULADO':'Aparelho: LIVRE'):'Aparelho: —';
...
function render(s={}){
  lastState=s;const online=fresh(s),platform=s.platformName||'Plataforma',age=s.lastSeen?Date.now()-s.lastSeen:null,net=s.diagnostics?.network||{},dom=s.diagnostics?.domCatalog||{},ws=Number(net.connections?.ws)||0,assetCount=s.marketCatalog?.assets?.length||Number(net.candidateCount)||Number(dom.assetCount)||0,structured=!!s.capabilities?.structuredQuotes,quality=Number(s.telemetry?.feedQuality)||0,licensed=activeLicense(s),telemetryRecent=s.telemetry?.lastSync&&Date.now()-s.telemetry.lastSync<20000;
...
async function getState(){try{render(await chrome.runtime.sendMessage({type:'ATS_GET_STATE'}))}catch{render({})}}
...
async function ensureBackendPermission(){const{settings={}}=await chrome.storage.local.get('settings');const base=settings.apiBase||PUBLIC_API;try{const u=new URL(base),origin=`${u.origin}/*`,has=await chrome.permissions.contains({origins:[origin]});if(has)return{ok:true};const granted=await chrome.permissions.request({origins:[origin]});return{ok:granted,error:granted?null:'permission_denied'}}catch{return{ok:false,error:'invalid_backend_url'}}}
