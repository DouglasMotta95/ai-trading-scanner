...
const EXTENSION_LATEST_VERSION = String(process.env.EXTENSION_LATEST_VERSION || '0.11.47').trim();
...
function licenseInput(b) { if (!isObj(b) || !str(b.licenseKey, 96) || !str(b.installationId, 128)) return null; return { licenseKey: b.licenseKey.trim().toUpperCase(), installationId: b.installationId, version: typeof b.version === 'string' ? b.version.slice(0, 32) : null, type: typeof b.type === 'string' ? b.type.slice(0, 32) : null, signalId: typeof b.signalId === 'string' ? b.signalId.trim().slice(0, 96) || null : null }; }
function licenseCheck(p, { consume = false } = {}) { const license = licenses.get(p.licenseKey); if (!license) return { status: 404, error: 'license_not_found' }; if (license.status !== 'active') return { status: 403, error: 'license_inactive' }; if (license.expiresAt && Date.parse(license.expiresAt) <= now()) return { status: 403, error: 'license_expired' }; const plan = planOf(license.plan); license.devices ??= []; license.deviceHistory ??= []; let device = license.devices.find(x => x.installationId === p.installationId); if (!device) { if (license.devices.length >= 1) { security('device_blocked', license, { installationId: p.installationId, boundInstallationId: license.devices[0]?.installationId || '', version: p.version || null }); return { status: 403, error: 'device_locked', license: publicLicense(license) }; } device = { installationId: p.installationId, createdAt: now(), lastSeen: now(), version: p.version || null }; license.devices.push(device); audit('device_bound', { licenseKey: license.key, customerName: license.customerName, plan: license.plan, installationId: p.installationId }); } else { device.lastSeen = now(); if (p.version) device.version = p.version; } license.usage ??= {}; license.consumedSignals ??= {}; const day = today(), used = Number(license.usage[day] || 0), usedTotal = totalUsage(license); if (consume && p.type === 'signal') { if (p.signalId && license.consumedSignals[p.signalId]) { license.updatedAt = now(); saveLicenses(); return { status: 200, license: publicLicense(license), usageDuplicate: true }; } if (plan.totalSignals != null && usedTotal >= plan.totalSignals) return { status: 429, error: 'trial_limit_reached', license: publicLicense(license) }; if (plan.dailySignals != null && used >= plan.dailySignals) return { status: 429, error: 'daily_limit_reached', license: publicLicense(license) }; license.usage[day] = used + 1; if (p.signalId) { license.consumedSignals[p.signalId] = { at: now(), day }; const keys = Object.keys(license.consumedSignals); if (keys.length > 2000) { keys.sort((a, b) => Number(license.consumedSignals[a]?.at || 0) - Number(license.consumedSignals[b]?.at || 0)); for (const key of keys.slice(0, keys.length - 2000)) delete license.consumedSignals[key]; } } } license.updatedAt = now(); saveLicenses(); return { status: 200, license: publicLicense(license), usageDuplicate: false }; }
...
function recordSignal(evt) { const d = evt.data || {}, direction = text(d.direction || '', 8).toUpperCase(), entryPrice = num(d.entryPrice); if (!['BUY', 'SELL'].includes(direction) || !text(d.asset || '', 64) || entryPrice == null) return; const signalId = text(d.signalId || '', 96) || crypto.randomUUID(); if (signalHistory.some(x => x.signalId === signalId)) return; const durationMs = parseDuration(d), entryAt = num(d.entryTime) ?? evt.at; signalHistory.unshift({ signalId, installationId: evt.installationId, licenseKey: evt.licenseKey, customerName: evt.customerName, plan: evt.plan, platformId: text(d.platformId || '', 64), platformName: text(d.platformName || '', 80), asset: text(d.asset || '', 64), direction, entryPrice, score: clamp(num(d.score) || 0, 0, 100), grade: text(d.grade || '', 24), timeframe: text(d.timeframe || '', 24), expiration: text(d.expiration || '', 24), durationMs, regime: text(d.regime || '', 40), confirmations: text(d.confirmations || '', 32), feedQuality: clamp(num(d.feedQuality) || 0, 0, 100), entryAt, expiresAt: entryAt + durationMs, outcome: 'pending', exitPrice: null, resolvedAt: null, resultSource: null }); signalHistory = signalHistory.slice(0, 3000); persistTelemetry(); }
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (pathname === '/download/extension' && ['GET', 'HEAD'].includes(req.method)) return serveExtensionPackage(res) || json(req, res, 404, { error: 'extension_package_missing' });
