...
async function sendVerification(req, account) { const token = issueToken('email-verify', { uid: account.id, email: account.email }, 24 * 3600000), url = `${baseUrl(req)}/?verify=${encodeURIComponent(token)}`; if (!RESEND_API_KEY || !EMAIL_FROM) return { sent: false }; const r = await fetch('https://api.resend.com/emails', { method: 'POST', headers: { authorization: `Bearer ${RESEND_API_KEY}`, 'content-type': 'application/json' }, body: JSON.stringify({ from: EMAIL_FROM, to: [account.email], subject: 'Confirme seu e-mail • AI Trading Scanner', html: `<div style="font-family:Arial,sans-serif;background:#07111c;color:#eef7ff;padding:32px"><h2>Confirme seu e-mail</h2><p>Olá ${htmlEscape(account.name || 'trader')}, confirme seu cadastro para liberar seu teste do AI Trading Scanner.</p><p><a href="${htmlEscape(url)}" style="display:inline-block;background:#44e0b5;color:#04120e;padding:14px 20px;border-radius:10px;text-decoration:none;font-weight:700">CONFIRMAR E-MAIL</a></p><p>Esse link expira em 24 horas.</p></div>` }) }); return { sent: r.ok }; }
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (pathname === '/v1/customer/resend-verification' && req.method === 'POST') { if (!rate(req, 'resend', 5, 3600000)) return json(req, res, 429, { error: 'too_many_attempts' }); const b = await readBody(req), email = cleanEmail(b.email); if (!isObj(b) || !only(b, ['email']) || !validEmail(email)) return json(req, res, 422, { error: 'invalid_email' }); const account = customerState.accounts.find(x => x.email === email); if (!account || account.emailVerified) return json(req, res, 200, { ok: true }); const delivery = await sendVerification(req, account); return json(req, res, 200, { ok: true, emailDeliveryConfigured: delivery.sent }); }
