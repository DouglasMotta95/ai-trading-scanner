...
function tokenSecret(label) {
  return crypto.createHash('sha256').update(`${label}:${ADMIN_KEY || API_KEY || 'dev-only-change-me'}`).digest();
}
...
function createLicense({ customerName = '', email = '', plan = 'starter', days }) { if (!PLANS[plan]) return null; const p = planOf(plan), createdAt = now(), validDays = Math.max(1, Number(days || p.defaultDays || 30)); const license = { key: newLicenseKey(), plan, status: 'active', customerName: String(customerName).slice(0, 120), email: String(email).slice(0, 160), createdAt, updatedAt: createdAt, expiresAt: new Date(createdAt + validDays * 86400000).toISOString(), devices: [], deviceHistory: [], usage: {}, priceSnapshot: cleanNumber(p.price, 0) }; licenses.set(license.key, license); saveLicenses(); audit(plan === 'trial' ? 'trial_created' : 'license_created', { licenseKey: license.key, customerName: license.customerName, plan, price: license.priceSnapshot }); return publicLicense(license); }
...
let liveEvents = [];
let signalHistory = [];
const clientTokenSecret = crypto.createHash('sha256').update(`ats-client:${API_KEY || ADMIN_KEY || 'development-only'}`).digest();
...
async function sendVerification(req, account) { const token = issueToken('email-verify', { uid: account.id, email: account.email }, 24 * 3600000), url = `${baseUrl(req)}/?verify=${encodeURIComponent(token)}`; if (!RESEND_API_KEY || !EMAIL_FROM) return { sent: false }; const r = await fetch('https://api.resend.com/emails', { method: 'POST', headers: { authorization: `Bearer ${RESEND_API_KEY}`, 'content-type': 'application/json' }, body: JSON.stringify({ from: EMAIL_FROM, to: [account.email], subject: 'Confirme seu e-mail • AI Trading Scanner', html: `<div style="font-family:Arial,sans-serif;background:#07111c;color:#eef7ff;padding:32px"><h2>Confirme seu e-mail</h2><p>Olá ${account.name || 'trader'}, confirme seu cadastro para liberar seu teste do AI Trading Scanner.</p><p><a href="${url}" style="display:inline-block;background:#44e0b5;color:#04120e;padding:14px 20px;border-radius:10px;text-decoration:none;font-weight:700">CONFIRMAR E-MAIL</a></p><p>Esse link expira em 24 horas.</p></div>` }) }); return { sent: r.ok }; }
...
async function processPayment(paymentId) { if (!PAYMENTS_CONFIGURED) return; const r = await fetch(`https://api.mercadopago.com/v1/payments/${encodeURIComponent(paymentId)}`, { headers: { authorization: `Bearer ${MP_ACCESS_TOKEN}` } }); if (!r.ok) return; const payment = await r.json(), order = customerState.orders.find(o => o.id === payment.external_reference); if (!order) return; order.paymentId = String(payment.id); order.paymentStatus = payment.status; order.updatedAt = now(); if (payment.status === 'approved' && order.status !== 'approved') { const account = customerState.accounts.find(x => x.id === order.accountId); if (!account) return; const license = grantPaidEntitlement(account, order.plan, order.plan === 'lifetime' ? LIFETIME_DAYS : 30); order.status = 'approved'; order.licenseKey = license.key; order.approvedAt = now(); } else if (payment.status !== 'approved') order.status = payment.status || 'pending'; saveCustomerState(); }
...
function serveAdminIndex(res) { try { let html = fs.readFileSync(path.join(ADMIN_DIR, 'index.html'), 'utf8'); const headExtras = `<link rel="stylesheet" href="/admin/crm.css?v=${VERSION}"><link rel="stylesheet" href="/admin/live.css?v=${VERSION}">`, bodyExtras = `<script src="/admin/crm.js?v=${VERSION}"></script><script src="/admin/live.js?v=${VERSION}"></script>`; if (!html.includes('/admin/crm.css')) html = html.replace('</head>', `${headExtras}</head>`); if (!html.includes('/admin/crm.js')) html = html.replace('</body>', `${bodyExtras}</body>`); else if (!html.includes('/admin/live.js')) html = html.replace('</body>', `<script src="/admin/live.js?v=${VERSION}"></script></body>`); html = html.replace(/v=0\.\d+\.\d+/g, `v=${VERSION}`); res.writeHead(200, { 'content-type': 'text/html; charset=utf-8', 'cache-control': 'no-store, max-age=0' }); res.end(html); return true; } catch { return false; } }
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    const adminAssets = { 'app.js': 'text/javascript; charset=utf-8', 'ops.js': 'text/javascript; charset=utf-8', 'crm.js': 'text/javascript; charset=utf-8', 'live.js': 'text/javascript; charset=utf-8', 'styles.css': 'text/css; charset=utf-8', 'ops.css': 'text/css; charset=utf-8', 'crm.css': 'text/css; charset=utf-8', 'live.css': 'text/css; charset=utf-8' }, adminAsset = pathname.match(/^\/admin\/([^/]+)$/);
    ...
    if (pathname === '/v1/customer/checkout' && req.method === 'POST') { const account = accountFromReq(req); if (!account) return json(req, res, 401, { error: 'unauthorized' }); const b = await readBody(req), out = await checkoutPreference(req, account, String(b.plan || '')); return json(req, res, 201, { ok: true, ...out }); }
    if (pathname === '/v1/payments/mercadopago/webhook' && req.method === 'POST') { if (!PAYMENTS_CONFIGURED) return json(req, res, 503, { error: 'payment_not_configured' }); const b = await readBody(req).catch(() => ({})), paymentId = String(url.searchParams.get('data.id') || b?.data?.id || ''); if (!mpSignatureValid(req, url)) return json(req, res, 401, { error: 'invalid_signature' }); const eventKey = String(b?.id || `${paymentId}:${b?.action || ''}`); if (eventKey && customerState.paymentEvents.includes(eventKey)) return json(req, res, 200, { ok: true, duplicate: true }); if (eventKey) { customerState.paymentEvents.unshift(eventKey); customerState.paymentEvents = customerState.paymentEvents.slice(0, 1000); saveCustomerState(); } if (paymentId) processPayment(paymentId).catch(e => console.error('payment webhook', e?.message || e)); return json(req, res, 200, { ok: true }); }
    ...
    if (pathname === '/v1/admin/auth/login' && req.method === 'POST') { if (!ADMIN_KEY) return json(req, res, 503, { error: 'admin_key_not_configured' }); const b = await readBody(req); if (!safeEq(b.adminKey, ADMIN_KEY)) return json(req, res, 401, { error: 'unauthorized' }); audit('admin_login'); return json(req, res, 200, { ok: true, expiresInDays: ADMIN_SESSION_DAYS }, { 'set-cookie': adminSessionCookie(req) }); }
