...
test('reconnect injects observable Quetta recovery before ordinary live readers', () => {
  ...
  const bootIndex = injector.indexOf("'src/content/runtime-boot-probe.js'");
  const bootstrapIndex = injector.indexOf("'src/content/page-world-bootstrap.js'");
  ...
  assert.ok(bootstrapIndex > bootIndex);
  assert.ok(topIndex > bootstrapIndex);
  assert.ok(recoveryIndex > topIndex);
  assert.ok(focusIndex > recoveryIndex);
  assert.match(injector, /executeScriptCompat/);
  assert.match(injector, /chrome\.scripting\.executeScript\(details, callback\)/);
  assert.match(injector, /returned && typeof returned\.then === 'function'/);
  assert.match(injector, /default-world-fallback/);
  assert.match(injector, /runtimeInjection/);
});

test('opaque recovery accepts Quetta sender quirks without opening the proxy to normal pages', () => {
  ...
  assert.match(proxy, /!senderUrl/);
  assert.match(proxy, /sender\.origin \|\| ''\) === 'null'/);
  assert.match(proxy, /!casaHost\(topHost\)/);
  assert.match(proxy, /!senderOpaque/);
  assert.doesNotMatch(proxy, /frameId === 0/);
...
test('opaque recovery recognizes standalone instruments without inventing a new signal engine', () => {
  ...
  assert.match(background, /background-runtime-telemetry\.js/);
...
test('page-world fallback is packaged for browsers that reject explicit MAIN world injection', () => {
  const manifest = JSON.parse(read('manifest.json'));
  const bootstrap = read('src/content/page-world-bootstrap.js');
  const sentinel = read('src/content/page-world-sentinel.js');
  const resources = manifest.web_accessible_resources?.flatMap(row => row.resources || []) || [];
  assert.equal(manifest.version, '0.11.10');
  assert.ok(resources.includes('src/content/page-world-sentinel.js'));
  assert.ok(resources.includes('src/content/network-probe.js'));
  assert.ok(resources.includes('src/content/worker-probe.js'));
  assert.match(bootstrap, /runtime\.getURL\(file\)/);
  assert.match(bootstrap, /page-world-confirmed/);
  assert.match(bootstrap, /ATS_RUNTIME_BOOT/);
  assert.match(sentinel, /ATS_PAGE_WORLD_SENTINEL/);
});
