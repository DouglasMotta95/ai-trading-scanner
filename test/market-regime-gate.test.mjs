...
test('market regime classifies the fixture as range', () => {
  ...
  const regime = marketRegime(candles.slice(0, -1));
...
test('telemetry serializes the market regime type instead of sending the full object', () => {
  const telemetry = fs.readFileSync(new URL('../src/services/telemetry.js', import.meta.url), 'utf8');
  assert.match(telemetry, /regime: state\.signal\?\.regime\?\.type \|\| state\.signal\?\.regime \|\| null/);
});
