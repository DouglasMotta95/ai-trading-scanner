...
test('consolidated backend flows from license activation to resolved live operation', { timeout: 20000 }, async () => {
  const dataDir = await mkdtemp(path.join(os.tmpdir(), 'ats-consolidated-'));
  ...
  try {
    ...
    assert.equal(healthJson.version, '0.9.4');
    ...
    const activate = await fetch(`${base}/v1/license/activate`, {
      ...
      body: JSON.stringify({ licenseKey: created.license.key, installationId, version: '0.9.4' })
    });
    ...
    const secondDevice = await fetch(`${base}/v1/license/activate`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ licenseKey: created.license.key, installationId: 'second-installation', version: '0.9.4' })
    });
    ...
    const heartbeat = await fetch(`${base}/v1/client/heartbeat`, {
      method: 'POST',
      headers: auth,
      body: JSON.stringify({ platformId: 'casatrade', platformName: 'CasaTrade', connection: 'online', scanning: true, asset: 'EUR/USD', timeframe: 'S5', expiration: '5s', price: 1.1000, signalState: 'CONFIRM', direction: 'BUY', score: 84, confirmations: '6 / 7', feedQuality: 96, structured: true, version: '0.9.4' })
    });
    ...
    const resolve = await fetch(`${base}/v1/client/heartbeat`, {
      method: 'POST',
      headers: auth,
      body: JSON.stringify({ platformId: 'casatrade', platformName: 'CasaTrade', connection: 'online', scanning: true, asset: 'EUR/USD', timeframe: 'S5', expiration: '5s', price: 1.1010, signalState: 'SEARCHING', score: 0, feedQuality: 96, structured: true, version: '0.9.4' })
    });
